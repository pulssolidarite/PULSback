...
class Customer(models.Model):
    login_user = models.OneToOneField("fleet.User", on_delete=models.PROTECT, related_name="login_customer", null=True) # User to log into the customer space
...
class User(AbstractUser):
    ...
    customer = models.ForeignKey(Customer, on_delete=models.CASCADE, related_name="users", null=True) # This field is for terminal users, to identify the customer who own the terminal. TODO this field should really be moved to Terminal model. User should be referenced by other objects and not reference other objects.
    is_admin = models.BooleanField(default=False) # TODO this field is used in Hera, but does not seem to have any purpose, should be removed
